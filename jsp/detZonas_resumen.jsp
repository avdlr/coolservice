...
<%@page import="servlet.DetZonas_ObtieneRegistros"%>
...
<script type="text/javascript">	
		...
		String mensaje="";
		DetZonas_ObtieneRegistros objConf = new DetZonas_ObtieneRegistros();
	...
	%>
	...
	
		{
			if (paginaAg < totalPaginasAg)
			{
				...
				$("#divresumenListaBlanca").load('<%=request.getContextPath()%>'+'/jsp/detZonas_resumen.jsp?'+parametros);
			}
	...
	$(window).resize(function() {     
		$("#dialogoDetalle").dialog("option", "position", "center"); 
			...
			function limpiarFiltros()
			{
				limpia_Filtros();
				$("#divresumenListaBlanca").html('<img style=" margin-left:0%; margin-top:5%; width:100px;" src="../img/circulo.gif"></img>');
				$("#divresumenListaBlanca").load('<%=request.getContextPath()%>'+'/jsp/detZonas_resumen.jsp?');
			}
		...
		{     
			...
			$("#divAgregarRegistro").load('<%=request.getContextPath()%>' + '/jsp/detZonas_altamodificacion.jsp?accion='+accion+'&valZona='+valZona+'&usuarioLogin='+'<%=usuarioLogin%>');
...
<!-- 						<th align='center' style="width: 80px">Usuario Alta</th> -->
				...
				<%
					DetZonas_Resumen objResumenConfiguracion = new DetZonas_Resumen();
									 			int i;
									 			String bgcolor = "";
													for (i = 0; i < lista_resumenConfiguracion.size(); i++) {
														objResumenConfiguracion = (DetZonas_Resumen)lista_resumenConfiguracion.get(i);
														if( bgcolor.equals("") ){
														     bgcolor = "odd";
				...
				<tbody>
					...
					<TR valign="middle" class="<%=bgcolor%>">
						<TD align="left" id="<%=bgcolor%>">&nbsp;<%=objResumenConfiguracion.getClaveDetZona()%></TD>
...
<%-- 						<TD align="left" id="<%=bgcolor%>">&nbsp;<%=objResumenConfiguracion.getCreacionUsuario()%></TD> --%>
						<TD align="left" id="<%=bgcolor%>">&nbsp;<%=objResumenConfiguracion.getEstatusUsuario()%></TD>
							...
							<a
								onclick="altaCambioRegistro('M','<%=objResumenConfiguracion.getClaveDetZona().trim()%>')"
								title="Editar Registro"><img alt="Editar Registro"
									src="../img/editar5.png" style="cursor: pointer;"></a> 
									<a
								onclick="eliminarRegistro('<%=objResumenConfiguracion.getClaveDetZona().trim()%>')"
