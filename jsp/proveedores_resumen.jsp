...
<%@page import="servlet.Proveedores_ObtieneRegistros"%>
...
<script type="text/javascript">	
		...
		String mensaje="";
		Proveedores_ObtieneRegistros objConf = new Proveedores_ObtieneRegistros();
		...
		ArrayList lista_resumenConfiguracion = new ArrayList();
	  	lista_resumenConfiguracion = objConf.obtieneResConf(valProveedor,Integer.parseInt(PAGINAAg));
		String numRegistrosAg = String.valueOf(objConf.obtieneTotalReg(valProveedor));
	...
	%>
	...
	
		{
			if (paginaAg < totalPaginasAg)
			{
				...
				if(valProveedor!="") { parametros+="&valProveedor="+escape(valProveedor); }
	...
	$(window).resize(function() {     
		$("#dialogoDetalle").dialog("option", "position", "center"); 
			...
			function limpiarFiltros()
			{
				limpia_Filtros();
				$("#divresumenListaBlanca").html('<img style=" margin-left:0%; margin-top:5%; width:100px;" src="../img/circulo.gif"></img>');
				$("#divresumenListaBlanca").load('<%=request.getContextPath()%>'+'/jsp/proveedores_resumen.jsp?');
			}
		 
		function altaCambioRegistro(accion, valProveedor)
		{     
			...
			$("#divAgregarRegistro").load('<%=request.getContextPath()%>' + '/jsp/proveedores_altamodificacion.jsp?accion='+accion+'&valProveedor='+valProveedor+'&usuarioLogin='+'<%=usuarioLogin%>');
			$("#divAgregarRegistro").dialog('open');
		}
		
		function eliminarRegistro( claveEmpresa) //estaba idregistro
		{
	...
	<div style="overflow: auto;">
		...
		<div style="overflow: auto;">
			<table class="tablaResumen" cellspacing="0" border="0">
				<thead>
					<tr>
						<th align='center' style="width: 20px">Clave</th>
						<th align='center' style="width: 120px">Nombre</th>
						<th align='center' style="width: 100px">Razon Social</th>
						<th align='center' style="width: 60px">RFC</th>
						<th align='center' style="width: 60px">Telefono</th>
<!-- 						<th align='center' style="width: 80px">Fecha Creacion</th> -->
<!-- 						<th align='center' style="width: 30px">Usuario Alta</th> -->
						<th align='center' style="width: 20px">Estatus</th>


						<th align='center' style="width: 20px">Acciones</th>
					</tr>
				</thead>

				<%
					Proveedores_Resumen objResumenConfiguracion = new Proveedores_Resumen();
									 			int i;
									 			String bgcolor = "";
													for (i = 0; i < lista_resumenConfiguracion.size(); i++) {
														objResumenConfiguracion = (Proveedores_Resumen)lista_resumenConfiguracion.get(i);
														if( bgcolor.equals("") ){
														     bgcolor = "odd";
				...
				<tbody>
					...
					<TR valign="middle" class="<%=bgcolor%>">
						<TD align="left" id="<%=bgcolor%>">&nbsp;<%=objResumenConfiguracion.getClaveProveedor()%></TD>
						<TD align="left" id="<%=bgcolor%>">&nbsp;<%=objResumenConfiguracion.getNombreProveedor()%></TD>
						<TD align="left" id="<%=bgcolor%>">&nbsp;<%=objResumenConfiguracion.getRazonSocial()%></TD>
						<TD align="left" id="<%=bgcolor%>">&nbsp;<%=objResumenConfiguracion.getRfcProveedor()%></TD>
						<TD align="left" id="<%=bgcolor%>">&nbsp;<%=objResumenConfiguracion.getTelefonoContacto()%></TD>
<%-- 						<TD align="left" id="<%=bgcolor%>">&nbsp;<%=objResumenConfiguracion.getFechaHoraEmpresa()%></TD> --%>
<%-- 						<TD align="left" id="<%=bgcolor%>">&nbsp;<%=objResumenConfiguracion.getCreacionEmpresa()%></TD> --%>
						<TD align="left" id="<%=bgcolor%>">&nbsp;<%=objResumenConfiguracion.getEstatusEmpresa()%></TD>
						
							<td align="center" id="<%=bgcolor%>">
							<a
								onclick="altaCambioRegistro('M','<%=objResumenConfiguracion.getClaveProveedor().trim()%>')"
								title="Editar Registro"><img alt="Editar Registro"
									src="../img/editar5.png" style="cursor: pointer;"></a> 
									<a
								onclick="eliminarRegistro('<%=objResumenConfiguracion.getClaveProveedor().trim()%>')"
