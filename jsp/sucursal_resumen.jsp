...
<%@page import="servlet.Sucursal_ObtieneRegistros"%>
...
<script type="text/javascript">	
		...
		String mensaje="";
		Sucursal_ObtieneRegistros objConf = new Sucursal_ObtieneRegistros();
		...
		ArrayList lista_resumenConfiguracion = new ArrayList();
	  	lista_resumenConfiguracion = objConf.obtieneResConf(valSucursal,Integer.parseInt(PAGINAAg));
		String numRegistrosAg = String.valueOf(objConf.obtieneTotalReg(valSucursal));
	...
	%>
	...
	
		{
			if (paginaAg < totalPaginasAg)
			{
				...
				if(valSucursal!="") { parametros+="&valSucursal="+escape(valSucursal); }
				...
				$("#divresumenListaBlanca").load('<%=request.getContextPath()%>'+'/jsp/sucursal_resumen.jsp?'+parametros);
			}
	...
	$(window).resize(function() {     
		$("#dialogoDetalle").dialog("option", "position", "center"); 
			...
			function limpiarFiltros()
			{
				limpia_Filtros();
				$("#divresumenListaBlanca").html('<img style=" margin-left:0%; margin-top:5%; width:100px;" src="../img/circulo.gif"></img>');
				$("#divresumenListaBlanca").load('<%=request.getContextPath()%>'+'/jsp/sucursal_resumen.jsp?');
			}
		 
		function altaCambioRegistro(accion, valSucursal)
		{     
			...
			$("#divAgregarRegistro").load('<%=request.getContextPath()%>' + '/jsp/sucursal_altamodificacion.jsp?accion='+accion+'&valSucursal='+valSucursal+'&usuarioLogin='+'<%=usuarioLogin%>');
			...
			<table class="tablaResumen" cellspacing="0" border="0">
				<thead>
					<tr>
						...
						<th align='center' style="width: 80px">Empresa</th>
						<th align='center' style="width: 80px">Nombre Sucursal</th>
						<th align='center' style="width: 80px">Nombre Corto Sucursal</th>
						<th align='center' style="width: 80px">Domicilio</th>
						<th align='center' style="width: 80px">Estado</th>
						<th align='center' style="width: 80px">Ciudad</th>
...
<!-- 						<th align='center' style="width: 80px">Usuario Alta</th> -->
				...
				<%
					Sucursal_Resumen objResumenConfiguracion = new Sucursal_Resumen();
									 			int i;
									 			String bgcolor = "";
													for (i = 0; i < lista_resumenConfiguracion.size(); i++) {
														objResumenConfiguracion = (Sucursal_Resumen)lista_resumenConfiguracion.get(i);
														if( bgcolor.equals("") ){
														     bgcolor = "odd";
				...
				<tbody>
					...
					<TR valign="middle" class="<%=bgcolor%>">
						<TD align="left" id="<%=bgcolor%>">&nbsp;<%=objResumenConfiguracion.getClave()%></TD>
						<TD align="left" id="<%=bgcolor%>">&nbsp;<%=objResumenConfiguracion.getEmpresa()%></TD>
						<TD align="left" id="<%=bgcolor%>">&nbsp;<%=objResumenConfiguracion.getNombre()%></TD>
						<TD align="left" id="<%=bgcolor%>">&nbsp;<%=objResumenConfiguracion.getNombreCorto()%></TD>
						<TD align="left" id="<%=bgcolor%>">&nbsp;<%=objResumenConfiguracion.getDomicilio()%></TD>
						<TD align="left" id="<%=bgcolor%>">&nbsp;<%=objResumenConfiguracion.getEstado()%></TD>
						<TD align="left" id="<%=bgcolor%>">&nbsp;<%=objResumenConfiguracion.getCiudad()%></TD>
...
<%-- 						<TD align="left" id="<%=bgcolor%>">&nbsp;<%=objResumenConfiguracion.getCreacionUsuario()%></TD> --%>
						<TD align="left" id="<%=bgcolor%>">&nbsp;<%=objResumenConfiguracion.getEstatusUsuario()%></TD>
							...
							<a
								onclick="altaCambioRegistro('M','<%=objResumenConfiguracion.getClave().trim()%>')"
								title="Editar Registro"><img alt="Editar Registro"
									src="../img/editar5.png" style="cursor: pointer;"></a> 
									<a
								onclick="eliminarRegistro('<%=objResumenConfiguracion.getClave().trim()%>')"
