...
<%@page import="servlet.Empresas_ObtieneRegistros"%>
...
<script type="text/javascript">	
		...
		String mensaje="";
		Empresas_ObtieneRegistros objConf = new Empresas_ObtieneRegistros();
		...
		ArrayList lista_resumenConfiguracion = new ArrayList();
	  	lista_resumenConfiguracion = objConf.obtieneResConf(valEmpresa,Integer.parseInt(PAGINAAg));
		String numRegistrosAg = String.valueOf(objConf.obtieneTotalReg(valEmpresa));
	...
	%>
	...
	
		{
			if (paginaAg < totalPaginasAg)
			{
				...
				if(valEmpresa!="") { parametros+="&valEmpresa="+escape(valEmpresa); }
	...
	$(window).resize(function() {     
		$("#dialogoDetalle").dialog("option", "position", "center"); 
			...
			function limpiarFiltros()
			{
				limpia_Filtros();
				$("#divresumenListaBlanca").html('<img style=" margin-left:0%; margin-top:5%; width:100px;" src="../img/circulo.gif"></img>');
				$("#divresumenListaBlanca").load('<%=request.getContextPath()%>'+'/jsp/empresas_resumen.jsp?');
			}
		 
		function altaCambioRegistro(accion, valEmpresa)
		{     
			...
			$("#divAgregarRegistro").load('<%=request.getContextPath()%>' + '/jsp/empresas_altamodificacion.jsp?accion='+accion+'&valEmpresa='+valEmpresa+'&usuarioLogin='+'<%=usuarioLogin%>');
			...
			<table class="tablaResumen" cellspacing="0" border="0">
				<thead>
					<tr>
						...
						<th align='center' style="width: 100px">Nombre Corto</th>
...
<!-- 						<th align='center' style="width: 30px">Usuario Alta</th> -->
				...
				<%
					Empresas_Resumen objResumenConfiguracion = new Empresas_Resumen();
									 			int i;
									 			String bgcolor = "";
													for (i = 0; i < lista_resumenConfiguracion.size(); i++) {
														objResumenConfiguracion = (Empresas_Resumen)lista_resumenConfiguracion.get(i);
														if( bgcolor.equals("") ){
														     bgcolor = "odd";
				...
				<tbody>
					...
					<TR valign="middle" class="<%=bgcolor%>">
						<TD align="left" id="<%=bgcolor%>">&nbsp;<%=objResumenConfiguracion.getClaveEmpresa()%></TD>
						<TD align="left" id="<%=bgcolor%>">&nbsp;<%=objResumenConfiguracion.getNombreEmpresa()%></TD>
						<TD align="left" id="<%=bgcolor%>">&nbsp;<%=objResumenConfiguracion.getNombreCortoEmpresa()%></TD>
						<TD align="left" id="<%=bgcolor%>">&nbsp;<%=objResumenConfiguracion.getRfcEmpresa()%></TD>
...
<%-- 						<TD align="left" id="<%=bgcolor%>">&nbsp;<%=objResumenConfiguracion.getCreacionEmpresa()%></TD> --%>
						<TD align="left" id="<%=bgcolor%>">&nbsp;<%=objResumenConfiguracion.getEstatusEmpresa()%></TD>
							...
							<a
								onclick="altaCambioRegistro('M','<%=objResumenConfiguracion.getClaveEmpresa().trim()%>')"
								title="Editar Registro"><img alt="Editar Registro"
									src="../img/editar5.png" style="cursor: pointer;"></a> 
									<a
								onclick="eliminarRegistro('<%=objResumenConfiguracion.getClaveEmpresa().trim()%>')"
