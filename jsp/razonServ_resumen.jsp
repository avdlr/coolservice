...
<%@page import="servlet.RazonServ_ObtieneRegistros"%>
...
<script type="text/javascript">	
		...
		String mensaje="";
		RazonServ_ObtieneRegistros objConf = new RazonServ_ObtieneRegistros();
	...
	$(window).resize(function() {     
		$("#dialogoDetalle").dialog("option", "position", "center"); 
			...
			function limpiarFiltros()
			{
				limpia_Filtros();
				$("#divresumenListaBlanca").html('<img style=" margin-left:0%; margin-top:5%; width:100px;" src="../img/circulo.gif"></img>');
				$("#divresumenListaBlanca").load('<%=request.getContextPath()%>'+'/jsp/razonServ_resumen.jsp?');
			}
		 
		function altaCambioRegistro(accion, valRazon)
		{     
			...
			$("#divAgregarRegistro").load('<%=request.getContextPath()%>' + '/jsp/razonServ_altamodificacion.jsp?accion='+accion+'&valRazon='+valRazon+'&usuarioLogin='+'<%=usuarioLogin%>');
			...
			<table class="tablaResumen" cellspacing="0" border="0">
				<thead>
					<tr>
						...
						<th align='center' style="width: 80px">Descripcion</th>
...
<!-- 						<th align='center' style="width: 80px">Usuario Alta</th> -->
				...
				<%
					RazonServ_Resumen objResumenConfiguracion = new RazonServ_Resumen();
									 			int i;
									 			String bgcolor = "";
													for (i = 0; i < lista_resumenConfiguracion.size(); i++) {
														objResumenConfiguracion = (RazonServ_Resumen)lista_resumenConfiguracion.get(i);
														if( bgcolor.equals("") ){
														     bgcolor = "odd";
				...
				<tbody>
					...
					<TR valign="middle" class="<%=bgcolor%>">
						<TD align="left" id="<%=bgcolor%>">&nbsp;<%=objResumenConfiguracion.getClaveRazonServ()%></TD>
...
<%-- 						<TD align="left" id="<%=bgcolor%>">&nbsp;<%=objResumenConfiguracion.getCreacionUsuario()%></TD> --%>
						<TD align="left" id="<%=bgcolor%>">&nbsp;<%=objResumenConfiguracion.getEstatusUsuario()%></TD>
							...
							<a
								onclick="altaCambioRegistro('M','<%=objResumenConfiguracion.getClaveRazonServ().trim()%>')"
								title="Editar Registro"><img alt="Editar Registro"
									src="../img/editar5.png" style="cursor: pointer;"></a> 
									<a
								onclick="eliminarRegistro('<%=objResumenConfiguracion.getClaveRazonServ().trim()%>')"
