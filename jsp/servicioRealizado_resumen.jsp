...
<%@page import="servlet.ServicioRealizado_ObtieneRegistros"%>
...
<script type="text/javascript">	
		...
		String mensaje="";
		ServicioRealizado_ObtieneRegistros objConf = new ServicioRealizado_ObtieneRegistros();
		...
		ArrayList lista_resumenConfiguracion = new ArrayList();
	  	lista_resumenConfiguracion = objConf.obtieneResConf(valZona,Integer.parseInt(PAGINAAg));
		String numRegistrosAg = String.valueOf(objConf.obtieneTotalReg(valZona));
	...
	%>
	...
	
		{
			if (paginaAg < totalPaginasAg)
			{
				...
				if(valZona!="") { parametros+="&valZona="+escape(valZona); }
				...
				$("#divresumenListaBlanca").load('<%=request.getContextPath()%>'+'/jsp/servicioRealizado_resumen.jsp?'+parametros);
			}
	...
	$(window).resize(function() {     
		$("#dialogoDetalle").dialog("option", "position", "center"); 
			...
			function limpiarFiltros()
			{
				limpia_Filtros();
				$("#divresumenListaBlanca").html('<img style=" margin-left:0%; margin-top:5%; width:100px;" src="../img/circulo.gif"></img>');
				$("#divresumenListaBlanca").load('<%=request.getContextPath()%>'+'/jsp/servicioRealizado_resumen.jsp?');
			}
		 
		function altaCambioRegistro(accion, valZona)
		{     
			...
			$("#divAgregarRegistro").load('<%=request.getContextPath()%>' + '/jsp/servicioRealizado_altamodificacion.jsp?accion='+accion+'&valZona='+valZona+'&usuarioLogin='+'<%=usuarioLogin%>');
			$("#divAgregarRegistro").dialog('open');
		}
		
		function eliminarRegistro( claveZona) //estaba idregistro
		{
			...
			<table class="tablaResumen" cellspacing="0" border="0">
				<thead>
					<tr>
						...
						<th align='center' style="width: 100px">Descripcion</th>
...
<!-- 						<th align='center' style="width: 30px">Usuario Alta</th> -->
				...
				<%
					ServicioRealizado_Resumen objResumenConfiguracion = new ServicioRealizado_Resumen();
									 			int i;
									 			String bgcolor = "";
													for (i = 0; i < lista_resumenConfiguracion.size(); i++) {
														objResumenConfiguracion = (ServicioRealizado_Resumen)lista_resumenConfiguracion.get(i);
														if( bgcolor.equals("") ){
														     bgcolor = "odd";
				...
				<tbody>
					...
					<TR valign="middle" class="<%=bgcolor%>">
						...
						<TD align="left" id="<%=bgcolor%>">&nbsp;<%=objResumenConfiguracion.getDescripcion()%></TD>
<%-- 						<TD align="left" id="<%=bgcolor%>">&nbsp;<%=objResumenConfiguracion.getFechaHoraZona()%></TD> --%>
<%-- 						<TD align="left" id="<%=bgcolor%>">&nbsp;<%=objResumenConfiguracion.getCreacionZona()%></TD> --%>
						<TD align="left" id="<%=bgcolor%>">&nbsp;<%=objResumenConfiguracion.getEstatusZona()%></TD>
