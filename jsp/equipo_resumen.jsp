...
<%@page import="servlet.Equipo_ObtieneRegistros"%>
...
<script type="text/javascript">	
		...
		String mensaje="";
		Equipo_ObtieneRegistros objConf = new Equipo_ObtieneRegistros();
		...
		ArrayList lista_resumenConfiguracion = new ArrayList();
	  	lista_resumenConfiguracion = objConf.obtieneResConf(valSucursal, valEmpresa, Integer.parseInt(PAGINAAg));
		String numRegistrosAg = String.valueOf(objConf.obtieneTotalReg(valSucursal,valEmpresa));
	...
	$(window).resize(function() {     
		$("#dialogoDetalle").dialog("option", "position", "center"); 
			...
			function limpiarFiltros()
			{
				limpia_Filtros();
				$("#divresumenListaBlanca").html('<img style=" margin-left:0%; margin-top:5%; width:100px;" src="../img/circulo.gif"></img>');
				$("#divresumenListaBlanca").load('<%=request.getContextPath()%>'+'/jsp/equipo_resumen.jsp?');
			}
		 
		function altaCambioRegistro(accion, valEquipo)
		{     
			...
			$("#divAgregarRegistro").load('<%=request.getContextPath()%>' + '/jsp/equipo_altamodificacion.jsp?accion='+accion+'&valEquipo='+valEquipo+'&usuarioLogin='+'<%=usuarioLogin%>');
			...
			<table class="tablaResumen" cellspacing="0" border="0">
				<thead>
					<tr>
						...
						<th align='center' style="width: 80px">Sucursal</th>
						<th align='center' style="width: 80px">Nombre</th>
						<th align='center' style="width: 80px">Marca</th>
						<th align='center' style="width: 80px">Modelo</th>
						<th align='center' style="width: 80px">Serie</th>
...
<!-- 						<th align='center' style="width: 80px">Usuario Alta</th> -->
				...
				<%
					Equipo_Resumen objResumenConfiguracion = new Equipo_Resumen();
									 			int i;
									 			String bgcolor = "";
													for (i = 0; i < lista_resumenConfiguracion.size(); i++) {
														objResumenConfiguracion = (Equipo_Resumen)lista_resumenConfiguracion.get(i);
														if( bgcolor.equals("") ){
														     bgcolor = "odd";
				...
				<tbody>
					...
					<TR valign="middle" class="<%=bgcolor%>">
						...
						<TD align="left" id="<%=bgcolor%>">&nbsp;<%=objResumenConfiguracion.getSucursal()%></TD>
						<TD align="left" id="<%=bgcolor%>">&nbsp;<%=objResumenConfiguracion.getNombre()%></TD>
						<TD align="left" id="<%=bgcolor%>">&nbsp;<%=objResumenConfiguracion.getMarca()%></TD>
						<TD align="left" id="<%=bgcolor%>">&nbsp;<%=objResumenConfiguracion.getModelo()%></TD>
						<TD align="left" id="<%=bgcolor%>">&nbsp;<%=objResumenConfiguracion.getSerie()%></TD>
