...
<%@page import="servlet.Zonas_ObtieneRegistros"%>
...
<script type="text/javascript">	
		...
		String mensaje="";
		Zonas_ObtieneRegistros objConf = new Zonas_ObtieneRegistros();
	...
	%>
	...
	
		{
			if (paginaAg < totalPaginasAg)
			{
				...
				$("#divresumenListaBlanca").load('<%=request.getContextPath()%>'+'/jsp/zonas_resumen.jsp?'+parametros);
			}
	...
	$(window).resize(function() {     
		$("#dialogoDetalle").dialog("option", "position", "center"); 
			...
			function limpiarFiltros()
			{
				limpia_Filtros();
				$("#divresumenListaBlanca").html('<img style=" margin-left:0%; margin-top:5%; width:100px;" src="../img/circulo.gif"></img>');
				$("#divresumenListaBlanca").load('<%=request.getContextPath()%>'+'/jsp/zonas_resumen.jsp?');
			}
		...
		{     
			...
			$("#divAgregarRegistro").load('<%=request.getContextPath()%>' + '/jsp/zonas_altamodificacion.jsp?accion='+accion+'&valZona='+valZona+'&usuarioLogin='+'<%=usuarioLogin%>');
			...
			<table class="tablaResumen" cellspacing="0" border="0">
				<thead>
					<tr>
						...
						<th align='center' style="width: 100px">Nombre</th>
...
<!-- 						<th align='center' style="width: 30px">Usuario Alta</th> -->
				...
				<%
					Zonas_Resumen objResumenConfiguracion = new Zonas_Resumen();
									 			int i;
									 			String bgcolor = "";
													for (i = 0; i < lista_resumenConfiguracion.size(); i++) {
														objResumenConfiguracion = (Zonas_Resumen)lista_resumenConfiguracion.get(i);
														if( bgcolor.equals("") ){
														     bgcolor = "odd";
				...
				<tbody>
					...
					<TR valign="middle" class="<%=bgcolor%>">
						<TD align="left" id="<%=bgcolor%>">&nbsp;<%=objResumenConfiguracion.getClaveZona()%></TD>
						<TD align="left" id="<%=bgcolor%>">&nbsp;<%=objResumenConfiguracion.getDescripcionZona()%></TD>
...
<%-- 						<TD align="left" id="<%=bgcolor%>">&nbsp;<%=objResumenConfiguracion.getCreacionZona()%></TD> --%>
						<TD align="left" id="<%=bgcolor%>">&nbsp;<%=objResumenConfiguracion.getEstatusZona()%></TD>
							...
							<a
								onclick="altaCambioRegistro('M','<%=objResumenConfiguracion.getClaveZona().trim()%>')"
								title="Editar Registro"><img alt="Editar Registro"
									src="../img/editar5.png" style="cursor: pointer;"></a> 
									<a
								onclick="eliminarRegistro('<%=objResumenConfiguracion.getClaveZona().trim()%>')"
